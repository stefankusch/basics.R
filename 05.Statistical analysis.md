...
Call:
glm(formula = value ~ sample, family = poisson, data = stat.data)
...
To compare mutant1 with mutant2 simply use the line `model_glm <- glm(value[5:12]~sample[5:12], family=poisson, data=stat.data)`
